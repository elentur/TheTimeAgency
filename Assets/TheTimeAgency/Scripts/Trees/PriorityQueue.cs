...
namespace Assets.TheTimeAgency.Scripts.Trees
{
    public class PriorityQueue<T> : IEnumerable<T> where T : System.IComparable<T>
    {
        ...
        public virtual void enqueue(T newNode)
        {
            ...
            {
                T tmpNode = queue[iParent];
                queue[iParent] = newNode;
                queue[iNewNode] = tmpNode;
                iNewNode = iParent;
                iParent = parent(iNewNode);
            }
        }
        ...
        public T dequeue()
        {
            ...
            while (iChild < n)
            {
                ...
                {
                    T tmpNode = queue[iChild];
                    queue[iChild] = queue[iParent];
                    queue[iParent] = tmpNode;
                }
                iParent = iChild;
                iChild = left(iParent);
            }
